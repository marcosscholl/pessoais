...
@Component("processaDLTSucessoCadastroContaSalarioConsumer")
@Slf4j
@AllArgsConstructor
public class ProcessaDLTSucessoCadastroContaSalarioConsumer implements Consumer<Message<String>> {
    ...
    @Value("${event.toggle.DLTSucessoCadastrosConsumer.enabled:true}")
    ...
    public void accept(Message<String> message) {
        ...
        if (FALSE.equalsIgnoreCase(eventToggle)) {
            log.info("Desabilitado consumo evento : ProcessaDLTSucessoCadastroContaSalarioConsumer : {}", cadastro.getCpf());
            acknowledgment.acknowledge();
            metricService.incrementCounter("dlt_event_sucessoCadastro_ignorado");
            return;
        }

        if (EVENT_CONFLICT.equalsIgnoreCase(String.valueOf(message.getHeaders().get(EVENT_HEADER_RESPONSE_ERROR_TYPE)))) {
            log.info("[{}] [{}] [CONFLICT] - Consumo de evento de DLT de Sucesso no Cadastro de Conta Salário finalizado por CONFLICT.",
                    transactionIdSolicitacao, transactionId);
            acknowledgment.acknowledge();
            metricService.incrementCounter("dlt_event_sucessoCadastro_conflict_bypass");
            return;
        }

        log.info("[{}] [{}] - Processando evento de DLT de sucesso de cadastro de Conta Salário.",
                transactionIdSolicitacao, transactionId);
        ...
        try {
            ...
            metricService.incrementCounter("dlt_event_sucessoCadastro_sucesso");
            log.info("[{}] [{}] - Consumo de evento de DLT de sucesso de cadastro de Conta Salário finalizado.",
                    transactionIdSolicitacao, transactionId);

        } catch (Exception e) {
            metricService.incrementCounter("dlt_event_sucessoCadastro_erro");
            log.error("[{}] [{}] - Consumo de evento de DLT de sucesso de cadastro de Conta Salário finalizado com erro. message: {}",
