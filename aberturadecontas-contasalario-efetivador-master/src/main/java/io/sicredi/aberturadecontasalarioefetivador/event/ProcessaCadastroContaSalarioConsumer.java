...
@Component("processaCadastroContaSalarioConsumer")
@Slf4j
@AllArgsConstructor
public class ProcessaCadastroContaSalarioConsumer implements Consumer<Message<String>> {
    ...
    @Value("${spring.cloud.stream.bindings.processaSucessoCadastroContaSalarioProducer-out-0.destination}")
    private final String sucessoCadastroTopic;
    @Value("${event.toggle.cadastroconsumer.enabled:true}")
    ...
    public void accept(Message<String> message) {
        ...
        if (FALSE.equalsIgnoreCase(eventToggle)) {
            log.info("Desabilitado consumo de evento : ProcessaCadastroContaSalarioConsumer : {}", cadastro.getCpf());
            acknowledgment.acknowledge();
            metricService.incrementCounter("event_cadastro_ignorado");
            return;
        }

        log.info("[{}] [{}] - Processando evento de cadastro de Conta Salário.",
        ...
        try {
            IdempotentAsyncRequest<Cadastro> request = IdempotentAsyncRequest
                    ...
                    .value(cadastro)
                    .headers(headers)
                    .transactionId(transactionId.substring(0,transactionId.length()-1).concat("2"))
                    .responseTopic(sucessoCadastroTopic)
                    .responseErrorTopic(erroCadastroTopic)
                    .build();

            contaSalarioService.processarSolicitacaoUnitaria(request);
            acknowledgment.acknowledge();

            metricService.incrementCounter("event_cadastro_sucesso");
            log.info("[{}] [{}] - Consumo do evento de cadastro de Conta Salário finalizado com sucesso",
                    transactionIdSolicitacao, transactionId);

        } catch (Exception e) {
            metricService.incrementCounter("event_cadastro_erro");
            log.error("[{}] [{}] - Consumo finalizado com erro no processo do evento de cadastro de Conta Salário. message: {}",
