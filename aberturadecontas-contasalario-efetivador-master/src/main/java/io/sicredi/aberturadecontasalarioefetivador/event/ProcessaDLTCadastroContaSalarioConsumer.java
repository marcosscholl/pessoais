...
@Component("processaDLTCadastroContaSalarioConsumer")
@Slf4j
@AllArgsConstructor
public class ProcessaDLTCadastroContaSalarioConsumer implements Consumer<Message<String>> {
    ...
    @Value("${event.toggle.DLTCadastrosConsumer.enabled:true}")
    ...
    public void accept(Message<String> message) {
        ...
        if (FALSE.equalsIgnoreCase(eventToggle)) {
            log.info("Desabilitado consumo evento : ProcessaDLTCadastroContaSalarioConsumer : {}", cadastro.getCpf());
            acknowledgment.acknowledge();
            metricService.incrementCounter("dlt_event_cadastro_ignorado");
            return;
        }

        if (EVENT_CONFLICT.equalsIgnoreCase(String.valueOf(message.getHeaders().get(EVENT_HEADER_RESPONSE_ERROR_TYPE)))) {
            log.info("[{}] [{}] [CONFLICT] - Consumo de evento de DLT de Cadastro de Conta Salário finalizado por CONFLICT.",
                    transactionIdSolicitacao, transactionId);
            acknowledgment.acknowledge();
            metricService.incrementCounter("dlt_event_cadastro_conflict_bypass");
            return;
        }

        log.info("[{}] [{}] - Processando evento de DLT de processamento de cadastro de Conta Salário.",
                transactionIdSolicitacao, transactionId);
        ...
        try {
            ...
            metricService.incrementCounter("dlt_event_cadastro_sucesso");
            log.info("[{}] [{}] - Consumo de evento de DLT de processamento de cadastro de Conta Salário finalizado.",
                    transactionIdSolicitacao, transactionId);

        } catch (Exception e) {
            metricService.incrementCounter("dlt_event_cadastro_erro");
            log.error("[{}] [{}] - Consumo de evento de DLT de processamento de cadastro de Conta Salário finalizado com erro. message: {}",
